...
#|-------------------------------------------------------------------------------
 | Name: John Graham
 | Pledge: Ipledge my honor that I have abided by the Stevens Honor System.
 |-------------------------------------------------------------------------------|#
...
(define (transitive-closure R)
  ...
  (define (tc-help e R)
    (cond
      ((equal? e 0) R)
      (else (union R (compose R (tc-help (- e 1) R))))))
  
  (tc-help (cardinality R) R))
...
(define (transitive? R)
  (cond
    ((equal? (cardinality R) 2)   (symmetric? R))
    ((equal? (cardinality R) 1)   (reflexive? R))
    (else (set-equal? R (transitive-closure R)))     ))
...
(define (involutive? n R)
  (and (symmetric? R)
       (injective? R)
       (subset? (converse R) (reflexive-closure n (converse R)))
       (subset? (id n) (reflexive-closure n (converse R)))     ))
