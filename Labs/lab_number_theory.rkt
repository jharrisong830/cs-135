...
(define (wj-solvable? cA cB g)
  (if (and (divides? (gcd cA cB) g) (or (>= cA g) (>= cB g)))
      #t
      #f))
...
(define (waterjug cA cB g)
  (define (help fA fB)
    (cond
      ((equal? fB g)      '())
      ((equal? fB cB)     (cons (list fA 0) (help fA 0)))
      ((equal? fA 0)      (cons (list cA fB) (help cA fB)))
      ((> (+ fA fB) cB)   (cons (list (- fA (- cB fB)) cB) (help (- fA (- cB fB)) cB)))
      ((<= (+ fA fB) cB)  (cons (list 0 (+ fA fB)) (help 0 (+ fA fB))))))

  (define (water-helper fA fB)
    (append (list (list 0 0) (list cA 0)) (help fA fB)))
