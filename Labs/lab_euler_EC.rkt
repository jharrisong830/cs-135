...
(define (all-even? G)
  ...
  (define (even-list L)
    (cond
      ((equal? L '()) 0)
      ((is-even? (degree G (car L))) (even-list (cdr L)))
      (else (+ 1 (even-list (cdr L))))))

  (equal? 0 (even-list (get-vertices G))))
...
(define (adjacent G v)
  (define (vList G)
    (cond
    ((equal? G '()) '())
    ((equal? (car (car G)) v)    (append (list (car (cdr (car G)))) (vList (cdr G))))
    ((equal? (car (cdr (car G))) v)   (append (list (car (car G))) (vList (cdr G))))
    (else (vList (cdr G)))))
...
;; 4 PTS
(define (connected? G)
  (define (connect-help G V)
    (cond
      ((equal? V '())    #t)
      ((equal? (usort (dfs G (car V))) (get-vertices G))    (connect-help G (cdr V)))
      (else #f)))

  (connect-help G (get-vertices G)))
